        ...
        if (args[0].equals("approve")) {
            ...
            if (owner != null) {
                if ((plot.settings.getAlias() != null) && !plot.settings.getAlias().equals("")) {
                    MainUtil.sendMessage(owner, "&7Your plot &a" + plot.id + " &7/ &a" + plot.settings.getAlias() + " has been approved!");
                } else {
                    MainUtil.sendMessage(owner, "&7Your plot &a" + plot.id + "&7 has been approved!");
                }
            }
                        ...
                        for (int i = 0; i < plots.size(); i++) {
                            if (plots.get(i).id.equals(plot.id) && plots.get(i).world.equals(world)) {
                                if (i < (plots.size() - 1)) {
                                    ...
                                    final Plot p2 = PlotSquared.getPlots(wrap.world).get(wrap.id);
